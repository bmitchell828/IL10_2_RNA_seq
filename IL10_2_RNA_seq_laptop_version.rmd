...
samples = read.csv("D:\\IL10 2 RNA seq\\metadata.csv", header = TRUE, stringsAsFactors=FALSE)
...
gtf="D:\\IL10 2 RNA seq\\GCF_000001635.27_GRCm39_genomic.gff.gz"   
...
files <- list.files(path = "D:\\IL10 2 RNA seq\\Salmon transcript quantificaion", pattern = ".tabular", full.names = T)
...
## Additionally, remove sample 'IEC_IL10KO_4' which appears to be an outlier based on exploratory analysis
cts <- round(txi$counts) # round raw counts
cts <- cts[,(-c(5,8,10,11,19,24,16,14,18) )]
samples <- samples[c("IEC_Cont_101","IEC_Cont_102","IEC_Cont_103","IEC_IL10KO_3","IEC_IL10KO_6","IEC_IL10KO_14","IEC_IL10KOZn_19","IEC_IL10KOZn_31"
...
IEC <- cts[,(1:8)]
IECsamples <- samples[1:8,]
IECsamples %>% 
  mutate(across(where(is.character), str_trim))

INT <- cts[,(9:15)]
INTsamples <- samples[9:15,]
...
png(filename="D:/IL10 2 RNA seq/DESeq_output/DESeq_sampleCorr_IEC_laptop.png", units = 'in', width = 10, height = 10, res = 250)
...
png(filename="D:/IL10 2 RNA seq/DESeq_output/DESeq_sampleCorr_INT_laptop.png", units = 'in', width = 10, height = 10, res = 250)
...
png(filename="D:/IL10 2 RNA seq/DESeq_output/DESeq_sampleDist_IEC_laptop.png", units = 'in', width = 10, height = 10, res = 250)
...
png(filename="D:/IL10 2 RNA seq/DESeq_output/DESeq_sampleDist_INT_laptop.png", units = 'in', width = 10, height = 10, res = 250)
pheatmap(sampleDistMatrix,
         clustering_distance_rows=sampleDists,
         clustering_distance_cols=sampleDists,
         annotation = treat_ann_INT,
...
png('D:/IL10 2 RNA seq/DESeq_output\\DESeq_PCA_IEC_laptop.png', units='in', width=7, height=5, res=250)
...
png('D:/IL10 2 RNA seq/DESeq_output/DESeq_PCA_INT_laptop.png', units='in', width=7, height=5, res=250)
ggplot(data, aes(PC1, PC2, color=Condition)) +
  geom_point(size=4, alpha = 0.85) +
  geom_text_repel(aes(label=name)) +
  theme_bw() +
...
# Write count table
write.csv(IEC.normalized.counts, file = 'D:/IL10 2 RNA seq/DESeq_output/DESeq2_RNAseq_IEC_normalized_counts.csv', quote = FALSE, sep = "\t", row.names = TRUE, col.names = NA)
...
## output file
write.csv(new.res, paste0('D:/IL10 2 RNA seq/DESeq_output/DESeq2_RNAseq_', condition1, '_vs_', condition2, '.csv'), quote=F, row.names = F)

write.csv(UP, paste0('D:/IL10 2 RNA seq/DESeq_output/UP_RNAseq_p0.05padj0.2', condition1, '_vs_', condition2, '.csv'), quote = FALSE, sep = "\t", row.names = F)

write.csv(DOWN, paste0('D:/IL10 2 RNA seq/DESeq_output/DOWN_RNAseq_p0.05padj0.2', condition1, '_vs_', condition2, '.csv'), quote = FALSE, sep = "\t", row.names = F)


## Volcano plot
png(paste0('D:/IL10 2 RNA seq/DESeq_output/MAPlot_FC_0.5_', condition1, '_vs_', condition2, '.png'), units = 'in', width = 6, height = 6, res = 250)
...
## Volcano plot
png(paste0('D:/IL10 2 RNA seq/DESeq_output/MAPlot_FC_0.05_', condition1, '_vs_', condition2, '.png'), units = 'in', width = 6, height = 6, res = 250)
g <- MAPlot(new.res) + theme(plot.margin=unit(c(1,1,1.5,1.2),"cm"))
g
dev.off()
...
## Volcano plot
png(paste0('D:/IL10 2 RNA seq/DESeq_output/MAPlot_FC_0.05', condition1, '_vs_', condition2, '.png'), units = 'in', width = 6, height = 6, res = 250)
g <- MAPlot(new.res) + theme(plot.margin=unit(c(1,1,1.5,1.2),"cm"))
g
dev.off()
...
## IL10 KO Zn vs IL10 KO

# Upregulated
UP <- read.csv("D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene\\ToppGene_UP_RNAseq_p0.05padj0.2IEC_IL10KOZn_vs_IEC_IL10KO.csv") 

# Biological Processes 
UP %>%
  select(1:5) %>%
  filter(Category == "GO: Biological Process") %>%
  mutate(neg_log_pval = -log(parse_number(pValue))) %>%
  arrange(desc(neg_log_pval)) %>%
  head(15) %>%
  mutate(name = str_c(Name, " (", ID, ")" )) %>%
  ggplot(mapping = aes(x = neg_log_pval, y = reorder(name, neg_log_pval))) +
...
ggsave(path = "D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene", filename ="GO results IL10KO vs IL10KOZn Upregulated.tiff", height=6, width=9, dpi = 600, device="tiff")
...
ggsave(path = "D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene", filename ="Disease signatures IL10KO vs IL10KOZn Upregulated.tiff", height=6, width=9, dpi = 600, device="tiff")

# Downregulated
DOWN <- read.csv("D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene\\ToppGene_DOWN_RNAseq_p0.05padj0.2IEC_IL10KOZn_vs_IEC_IL10KO.csv")
...
DOWN %>%
  ...
  labs(title = "Downregualted", x = "-log(adjusted p-value)", y = "")+
  scale_y_discrete(labels = function(x) str_wrap(x, width = 50)) + 
  geom_vline(xintercept = 3,linetype=2)

ggsave(path = "D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene", filename ="GO results IL10KO vs IL10KOZn Downregulated.tiff", height=6, width=9, dpi = 600, device="tiff")
...
ggsave(path = "D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene", filename ="Disease signatures IL10KO vs IL10KOZn Downregulated.tiff", height=6, width=9, dpi = 600, device="tiff")

### IL10 KO vs Control
# Upregulated

UP <- read.csv("D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene\\ToppGene_UP_RNAseq_p0.05padj0.2IEC_IL10KO_vs_IEC_WT.csv") 
...
ggsave(path = "D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene", filename ="GO results IL10KO vs WT Upregulated.tiff", height=6, width=9, dpi = 600, device="tiff")
...
ggsave(path = "D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene", filename ="Disease signatures IL10KO vs WT Upregulated.tiff", height=6, width=9, dpi = 600, device="tiff")

# Downregulated
DOWN <- read.csv("D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene\\ToppGene_DOWN_RNAseq_p0.05padj0.2IEC_IL10KO_vs_IEC_WT.csv")
...
ggsave(path = "D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene", filename ="GO results IL10KO vs WT Downregulated.tiff", height=6, width=9, dpi = 600, device="tiff")
...
ggsave(path = "D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene", filename ="Disease signatures IL10KO vs WT Downregulated.tiff", height=6, width=9, dpi = 600, device="tiff")
...
png(paste0('D:/IL10 2 RNA seq/DESeq_output/UP in IL10KO vs WT AND DOWN in IL10 KO Zn vs WT.png'), units = 'in', width = 15, height = 15, res = 500)
g <- plot(ven) + theme(plot.margin=unit(c(1,100,1.5,1.2),"cm"))
g
dev.off()
...
ven_list <- getSet(ven, subset = "Shared")
names(ven_list) <- c("Subset", "Gene")
write.csv(ven_list$Gene, file = "C:\\Users\\Blake\\Documents\\IL10_2_RNA_seq\\Overlapping Genes\\DOWN in IL10 KO vs WT & UP in IL10KO Zn vs WT.csv")
png(paste0('D:/IL10 2 RNA seq/DESeq_output/DOWN in IL10KO vs WT AND UP in IL10 KO Zn vs WT.png'), units = 'in', width = 15, height = 15, res = 500)
g <- plot(ven) + theme(plot.margin=unit(c(1,100,1.5,1.2),"cm"))
g
dev.off()
...
```{r}
## Graph GO results from ToppGene of reciprocally regulated Genes
## Read in GO results of reciprocally regulated genes
# First, Genes that are DOWN in IL10 KO vs WT and UP in IL10 KO + Zn vs IL10 KO
genes <- read.csv("D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene\\ToppGene DOWN in IL10KO and UP in IL10KO Zn.csv")
...
genes %>%
  ...
  labs(title = "Downregulated in IL10 KO vs WT and Upregulated in IL10KO + Zn vs IL10 KO", x = "-log(adjusted p-value)", y = "")+
  scale_y_discrete(labels = function(x) str_wrap(x, width = 50)) + 
  geom_vline(xintercept = 3,linetype=2)

ggsave(path = "D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene", filename ="GO results DOWN in IL10 KO and UP in IL10 KOZn.tiff", height=6, width=9, dpi = 600, device="tiff")
...
ggsave(path = "D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene", filename ="Disease signatures DOWN in IL10 KO and UP in IL10 KOZn.tiff", height=6, width=9, dpi = 600, device="tiff")

# Next, Genes that are UP in IL10 KO vs WT and DOWN in IL10 KO + Zn vs IL10 KO
genes <- read.csv("D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene\\ToppGene UP in IL10KO and DOWN in IL10KO Zn.csv")
...
genes %>%
  ...
  labs(title = "Upregulated in IL10 KO vs WT and Downregulated in IL10KO + Zn vs IL10 KO", x = "-log(adjusted p-value)", y = "")+
  scale_y_discrete(labels = function(x) str_wrap(x, width = 50)) + 
  geom_vline(xintercept = 3,linetype=2)

ggsave(path = "D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene", filename ="GO results UP in IL10 KO and DOWN in IL10 KOZn.tiff", height=6, width=9, dpi = 600, device="tiff")
...
ggsave(path = "D:\\IL10 2 RNA seq\\DESeq_output\\ToppGene", filename ="Disease signatures UP in IL10 KO and DOWN in IL10 KOZn.tiff", height=6, width=9, dpi = 600, device="tiff")
...
my_plot <- ggplot(data = gap_to_plot, mapping = aes(x = year, y = gdpPercap)) +
  geom_point() +
  labs(title = str_c(cntry, "GDP per cap.png", sep = " "))

ggsave(plot = my_plot, filename = str_c("plots/", cntry, "_GDP_per_cap.png"))
