...
# Write count table
write.csv(normalized.counts, file = 'DESeq_output/DESeq2_RNAseq_normalized_counts.csv', quote = FALSE, sep = "\t", row.names = TRUE, col.names = NA)
```
...
nrow(new.res[which(new.res$pvalue < 0.05 & new.res$padj < 0.2 & new.res$log2FoldChange > 0 & new.res$baseMean > 50),]) #2
nrow(new.res[which(new.res$pvalue < 0.05 & new.res$padj < 0.2 &  new.res$log2FoldChange < 0 & new.res$baseMean> 50),]) #18

UP <- as.data.frame(new.res[which(new.res$pvalue < 0.05 & new.res$padj < 0.2 &  new.res$log2FoldChange > 0 & new.res$baseMean > 50),])
UP$gene <- rownames(UP)
DOWN <- as.data.frame(new.res[which(new.res$pvalue < 0.05 & new.res$padj < 0.2 & new.res$log2FoldChange < 0 & new.res$baseMean > 50),])
...
full_list <- res$log2FoldChange
names(full_list) <- res$geneName
gene_list <- na.omit(full_list)
...
```{r}
## Differential analysis
res <- results(dds, contrast = c("Condition", "Control", "IL10KOZn"))
condition1 <- "IEC_Control"
condition2 <- "IEC_IL10KOZn"
...
## output file
write.csv(new.res, paste0('DESeq_output/DESeq2_RNAseq_', condition1, '_vs_', condition2, '.csv'), quote=F, row.names = F)
write.table(UP, paste0('DESeq_output/UP_RNAseq_p0.05padj0.2', condition1, '_vs_', condition2, '.txt'), quote = FALSE, sep = "\t", row.names = F)
write.table(DOWN, paste0('DESeq_output/DOWN_RNAseq_p0.05padj0.2', condition1, '_vs_', condition2, '.txt'), quote = FALSE, sep = "\t", row.names = F)
...
## Differential analysis
res <- results(dds, contrast = c("group", "Control", "Zn"))
condition1 <- "INT_IL10KO"
condition2 <- "INT_IL10KOZn"
